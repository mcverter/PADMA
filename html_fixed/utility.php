...
else if ($param=="ResetPassword")
{
  ...
  else
  {
    ...
    echo $mail_sent ? "A new password has been sent to your account  $temppass" : "Error: sending mail to the account $email";
    echo "</font>";
  }
}
...
{
  $cmdstr = "select distinct $param from FULL_V where RESTRICTED ='0' order by $param";
  $parsed = ociparse($db_conn, $cmdstr);
  ociexecute($parsed);
